...
Define transient injection semantics for memory lookup results. Memory context is appended to conversation history as a `_transient_memory` tagged system message, with automatic cleanup of previous transient messages to prevent stale accumulation.
