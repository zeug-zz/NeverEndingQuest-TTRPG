...
Encode blueprint handoff metadata (path, status, source-lock settings, artifact references) in `builder_input.json` so downstream packet builder stages can identify the handoff mode and refuse non-ready blueprint states.
