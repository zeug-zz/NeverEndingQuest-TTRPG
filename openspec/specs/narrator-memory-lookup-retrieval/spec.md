...
Define bounded entity timeline retrieval for narrator memory lookup. Memory events are retrieved from the memory DB via `get_entity_timeline()`, deduplicated by `event_id`, sorted by `retrieval_score`, and truncated to 8 events total with 150 character summaries.
