...
Define the `author` and `license` fields in `module_context.json` as the canonical copyright authority for each module. These fields are emitted as empty strings at generation time for the module developer to fill, and `module_context.json` is the correct location because it is a canonical (static, committed) artifact — unlike `module_plot.json` which is runtime-mutated state.
