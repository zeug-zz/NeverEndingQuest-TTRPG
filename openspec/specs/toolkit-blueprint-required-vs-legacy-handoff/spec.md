...
Distinguish accurate-ingest workspaces that require a ready blueprint from legacy workspaces that can fall back to the existing builder handoff path, so partial/missing blueprint artifacts in accurate-ingest mode fail closed before executor invocation.
