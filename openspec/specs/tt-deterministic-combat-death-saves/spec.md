...
Define deterministic Python-owned death-save prompting, parsing, persistence, and PC-phase cadence for multi-PC combat.
...
#### Scenario: Death-save roll must come from the pending PC tab

- **GIVEN** Acheron is awaiting a death saving throw
- **AND** Merisiel's tab is selected for input
- **WHEN** Merisiel enters `3`
- **THEN** Python SHALL reject the input with `[skipTTS]` system-style guidance naming Acheron as the PC who must roll
- **AND** Python SHALL NOT apply `3` to Acheron's death saves
- **AND** the input SHALL NOT be sent to the LLM for interpretation

#### Scenario: Wrong-tab action input cannot satisfy a pending death save

- **GIVEN** Acheron is awaiting a death saving throw
- **AND** Merisiel's tab is selected for input
- **WHEN** Merisiel enters `/att goblin 17` or `I attack troll 7`
- **THEN** Python SHALL reject the input with `[skipTTS]` system-style guidance naming Acheron as the PC who must roll
- **AND** Python SHALL NOT parse any number in the input as Acheron's death-save roll
- **AND** the action SHALL NOT be applied
- **AND** the input SHALL NOT be sent to the LLM for interpretation
...
#### Scenario: Unrelated numeric text is not parsed as a death save

- **GIVEN** Acheron is awaiting a death saving throw
- **WHEN** Acheron enters `I attack troll 7` or `troll 7`
- **THEN** Python SHALL reject the input with death-save roll guidance
- **AND** Python SHALL NOT apply `7` as a death-save roll
- **AND** the input SHALL NOT be sent to the LLM for interpretation
...
#### Scenario: Multiple downed PCs form a deterministic PC-phase queue

- **GIVEN** Acheron and Merisiel are both at `0 HP` with unresolved death-save obligations
- **WHEN** combat enters the next `PC_PHASE`
- **THEN** Python SHALL select the first pending PC from deterministic party/order state
- **AND** only the selected pending PC's tab input SHALL be accepted for that roll
- **AND** after the selected PC resolves their death save, Python SHALL advance to the next pending PC before allowing normal actions or `/end`
- **AND** enemy/NPC batch processing SHALL NOT begin until the current PC-phase death-save queue is empty
...
#### Scenario: Other PCs can continue normally after death-save queue is clear

- **GIVEN** Acheron is at `0 HP`
- **AND** Acheron's current PC-phase death-save obligation has already been resolved
