...
Define system prompt and validation prompt contract entries for the `lookupMemory` action. The compressed and uncompressed system prompts must include `lookupMemory` in `@ACTIONS`, `@PARAMS`, a new `@MEMORY_LOOKUP` directive, and `@EXAMPLES`. Validation prompts must include lookupMemory validation rules. All additions must be ASCII-only.
