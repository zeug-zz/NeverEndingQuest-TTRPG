...
- Prefer a small helper `getAssetKey(assetType, assetId)` for consistent key formation.
- Keep the existing `viewAssetMedia(assetId, assetType, assetName, mediaType)` signature; resolve `assetId` and `assetType` from the qualified key when calling it.
- Keep qualified keys as a defensive UI invariant even after backend duplicate suppression is implemented.
