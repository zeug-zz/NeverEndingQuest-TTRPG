...
The homebrew/accurate-ingest review UI MUST NOT imply a module is ready for gameplay testing while validation, topology, artifact, publishability, report-agreement, or playable-publication blockers remain. The UI MUST distinguish playable publication from clean source-fidelity pass when final reconciliation accepts source-fidelity blockers.
...
#### Scenario: Reconciled playable module is not shown as source-fidelity pass

- **GIVEN** final reconciliation accepts editorial source-fidelity blockers
- **AND** playable-publication gates pass
- **WHEN** the GUI renders completion status
- **THEN** it SHALL show the module as playable or publication-ready
- **AND** it SHALL show source fidelity as reconciled/degraded rather than pass
- **AND** it SHALL NOT display generic support-link failure copy for the reconciled editorial blockers.
