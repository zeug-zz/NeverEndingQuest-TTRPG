# Spec: NPC Lovecraftian Depth

## Purpose

Ensure all 9 NPC entries in `module_context.json` carry sufficient Lovecraftian narrative depth for the LLM Narrator, including multi-playline role framing.
...
- Descriptions SHOULD use Lovecraftian sensory language (texture, scale, wrongness) rather than combat stats
- Role fields SHOULD use the format: "Primary Role / Secondary Interpretation / Hidden Truth"
- Faction alignment SHOULD reflect the NPC's relationship to the five playline truths
- Descriptions SHOULD be ASCII-only (no Unicode characters per repository policy)
